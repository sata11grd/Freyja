...
static sgx_status_t SGX_CDECL Enclave_frey_read_source_ocall(void* pms)
{
	ms_frey_read_source_ocall_t* ms = SGX_CAST(ms_frey_read_source_ocall_t*, pms);
	frey_read_source_ocall(ms->ms_ptr, ms->ms_len);
...
static const struct {
	...
	14,
	{
		...
		(void*)(uintptr_t)Enclave_frey_read_source_ocall,
...
sgx_status_t frey_test(sgx_enclave_id_t eid)
{
	sgx_status_t status;
