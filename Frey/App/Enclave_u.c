...
static sgx_status_t SGX_CDECL Enclave_cpy(void* pms)
{
	ms_cpy_t* ms = SGX_CAST(ms_cpy_t*, pms);
	cpy(ms->ms_indata);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_store_source_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave_cp_source_ocall(void* pms)
{
	ms_cp_source_ocall_t* ms = SGX_CAST(ms_cp_source_ocall_t*, pms);
	cp_source_ocall(ms->ms_ptr, ms->ms_len);
...
static const struct {
	...
	17,
	{
		...
		(void*)(uintptr_t)Enclave_cpy,
		(void*)(uintptr_t)Enclave_store_source_ocall,
		(void*)(uintptr_t)Enclave_cp_source_ocall,
